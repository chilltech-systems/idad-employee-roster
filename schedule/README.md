...
The production portal at https://idad-employee-report-directory.vercel.app now reads and writes its separate `employee_directory` database and syncs only this private copied schedule. Vercel runs `/api/cron/directory-sync` daily at `0 10 * * *` UTC (approximately 5 a.m. Central during daylight time, within Vercel's scheduling window). Administrators can use **Refresh roster** and **Sync dropdowns now** without waiting. Saving an employee alone does not immediately update dropdowns; use manual sync or wait for the daily cycle.
...
The private copy is the only Google write target. The POS roster reader uses readonly scope. Database writes stay in the four production portal collections; DirectoryDB, the original schedule, live reporting workflows and bound scripts remain unchanged.
